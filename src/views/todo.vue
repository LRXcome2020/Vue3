<!--
 * @,@Author: ,: your name
 * @,@Date: ,: 2020-11-05 14:08:15
 * @,@LastEditTime: ,: 2020-11-05 15:42:22
 * @,@LastEditors: ,: Please set LastEditors
 * @,@Description: ,: In User Settings Edit
 * @,@FilePath: ,: \goods\projectName\src\views\todo.vue
 -->
<template>
...
export default {
  setup() {
    const todo = ref(""); //定义变量
    const time = `${new Date().getFullYear()}-${new Date().getMonth()}-${new Date().getDate()}`; //定义时间
    console.log("时间", time);
    const addTodo = (value) => {
      if (!value) {
        alert("请输入待办事项");
